...
export default async function ProjectsDashboardPage({ params }: { params: Promise<{ lang: Language }> }) {
      ...
      <div className="h-svh overflow-hidden lg:p-2 w-full">
        <div className="lg:border lg:rounded-md overflow-hidden flex flex-col h-full w-full bg-background">
          ...
          <main className="w-full flex-1 overflow-auto p-4">
            <div className="mb-4">
              <h2 className="text-2xl font-semibold tracking-tight">{dict.sidebar.projects}</h2>
              <p className="text-muted-foreground">{dict.sidebar.projects} management.</p>
            </div>
            <ContentTable dict={dict as AdminDictionary} contentType="projects" lang={lang} />
          </main>
        </div>
      </div>
    </SidebarProvider>
