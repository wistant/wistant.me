...
export function Header({ post, lang, dict, readingTime }: HeaderProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <Link
        title={dict.blog?.backToAll || "Go back"}
        href="/blog"
      ...
      >
          <ArrowLeft/>
          <span>{dict.blog?.backToAll || "blog"}</span>
      </Link>
      ...
      <div className="flex items-center gap-3 text-xs font-mono text-neutral-500">
        <span><a href={siteConfig.links.x} className="text-foreground underline font-medium">@{siteConfig.name}</a></span>
        <span>|</span>
